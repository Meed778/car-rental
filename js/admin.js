...
const AdminApp = {
      ...
      <div class="admin-table">
        ...
        ${cars.map(car => `
          <div class="admin-table-row">
            <div>
              ${car.image && car.image.startsWith("data:") 
                ? `<img src="${car.image}" alt="${car.brand}" class="admin-thumb">`
            ...
            <div class="admin-actions">
              <button class="admin-btn-icon edit" onclick="AdminApp.editCar('${car.id}')" title="تعديل">
                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"/></svg>
              </button>
              <button class="admin-btn-icon delete" onclick="AdminApp.deleteCar('${car.id}')" title="حذف">
  ...
  showCarForm(carData) {
    ...
    const car = carData || {
      ...
      seats: 5, doors: 4, pricePerDay: "", currency: "MAD",
      image: "", features: [], available: true, promo: false
    };
    ...
    document.getElementById("adminCarFormContent").innerHTML = `
      ...
      <div class="admin-form-grid">
        <div class="form-group">
          ...
          <input type="text" class="form-input" id="af_brand" value="${car.brand}" placeholder="مثال: Mercedes-Benz">
        </div>
        <div class="form-group">
          <label class="form-label">الموديل</label>
          <input type="text" class="form-input" id="af_model" value="${car.model}" placeholder="مثال: C300 AMG">
        </div>
        <div class="form-group">
          <label class="form-label">السنة</label>
          <input type="number" class="form-input" id="af_year" value="${car.year}">
        </div>
        <div class="form-group">
          <label class="form-label">الفئة</label>
          <select class="form-select" id="af_category">
            ${["luxury","suv","sport","economy"].map(c =>
              `<option value="${c}" ${car.category === c ? 'selected' : ''}>${
                {luxury:"فاخرة",suv:"SUV",sport:"رياضية",economy:"اقتصادية"}[c]
              }</option>`
            ).join('')}
          </select>
        </div>
        <div class="form-group">
          ...
          <select class="form-select" id="af_transmission">
        ...
        <div class="form-group">
          ...
          <select class="form-select" id="af_fuel">
            <option value="Gasoline" ${car.fuel === 'Gasoline' ? 'selected' : ''}>بنزين</option>
            <option value="Diesel" ${car.fuel === 'Diesel' ? 'selected' : ''}>ديزل</option>
            <option value="Hybrid" ${car.fuel === 'Hybrid' ? 'selected' : ''}>هايبرد</option>
            <option value="Electric" ${car.fuel === 'Electric' ? 'selected' : ''}>كهربائي</option>
          </select>
        </div>
        <div class="form-group">
          <label class="form-label">عدد المقاعد</label>
          <input type="number" class="form-input" id="af_seats" value="${car.seats}" min="2" max="9">
        </div>
        <div class="form-group">
          <label class="form-label">عدد الأبواب</label>
          <input type="number" class="form-input" id="af_doors" value="${car.doors}" min="2" max="6">
        </div>
        <div class="form-group">
          <label class="form-label">السعر لليوم</label>
          <input type="number" class="form-input" id="af_price" value="${car.pricePerDay}" placeholder="مثال: 850">
        </div>
        <div class="form-group">
          <label class="form-label">العملة</label>
          <select class="form-select" id="af_currency">
            <option value="MAD" ${car.currency === 'MAD' ? 'selected' : ''}>درهم (MAD)</option>
            <option value="EUR" ${car.currency === 'EUR' ? 'selected' : ''}>يورو (EUR)</option>
            <option value="USD" ${car.currency === 'USD' ? 'selected' : ''}>دولار (USD)</option>
          </select>
        </div>
        <div class="form-group">
          <label class="form-label">المميزات (واحد بكل سطر)</label>
          <textarea class="form-textarea" id="af_features" rows="4" placeholder="Apple CarPlay&#10;سقف بانورامي&#10;مقاعد جلد">${car.features.join("\n")}</textarea>
        </div>
        <div class="form-group" style="grid-column:span 2">
          ...
          <div class="admin-upload-area" id="adminUploadArea">
            ...
            <div id="adminUploadPlaceholder" style="${car.image && !car.image.startsWith('data:') ? 'display:none' : ''}">
              ...
              <p style="color:var(--gray-400);margin-top:8px">اختر صورة من جهازك</p>
              <span style="color:var(--gray-500);font-size:0.8rem">تنسيق JPG, PNG, WebP</span>
            </div>
            <div id="adminUploadPreview" style="${car.image && car.image.startsWith('data:') ? '' : 'display:none'};position:relative">
              <img id="af_imagePreview" src="${car.image && car.image.startsWith('data:') ? car.image : ''}" style="max-height:200px;border-radius:8px">
              <button type="button" onclick="AdminApp.removeUploadedImage()" style="position:absolute;top:-8px;left:-8px;width:28px;height:28px;border-radius:50%;background:#ef4444;color:#fff;display:flex;align-items:center;justify-content:center;font-size:1rem">&times;</button>
            </div>
          </div>
          <div style="display:flex;gap:12px;margin-top:8px;flex-wrap:wrap">
            <label class="btn-secondary" style="cursor:pointer;padding:8px 16px;font-size:0.85rem">
              ...
              رفع صورة
              <input type="file" accept="image/*" onchange="AdminApp.handleImageUpload(event)" style="display:none">
            </label>
            <button type="button" class="btn-secondary" style="padding:8px 16px;font-size:0.85rem" onclick="AdminApp.useUrlImage()">
              استخدام رابط URL
            </button>
          </div>
        ...
        <div class="form-group" style="grid-column:span 2">
          <div style="display:flex;gap:20px">
            <label style="display:flex;align-items:center;gap:8px;cursor:pointer">
              <input type="checkbox" id="af_available" ${car.available ? 'checked' : ''}>
              <span style="color:var(--gray-300)">متاحة للحجز</span>
            </label>
            <label style="display:flex;align-items:center;gap:8px;cursor:pointer">
              <input type="checkbox" id="af_promo" ${car.promo ? 'checked' : ''}>
              <span style="color:var(--gray-300)">عرض خاص</span>
            </label>
          </div>
        </div>
      </div>
      <div class="form-actions" style="margin-top:24px">
        <button class="btn-secondary" onclick="AdminApp.closeCarForm()">إلغاء</button>
        <button class="btn-primary" onclick="AdminApp.saveCar()">${isEdit ? 'حفظ التعديلات' : 'إضافة السيارة'}</button>
      </div>
    `;

    document.getElementById("adminUploadArea").addEventListener("click", () => {
      document.getElementById("af_imageInput").click();
    });
  ...
  useUrlImage() {
    const input = document.getElementById("af_imageUrl");
  ...
  saveCar() {
    ...
    const image = imgPreview?.dataset?.base64
      || document.getElementById("af_imageUrl")?.value?.trim()
      || "";

    const features = document.getElementById("af_features").value
      .split("\n")
      .map(f => f.trim())
      .filter(f => f);
    ...
    if (!car.brand || !car.model || car.pricePerDay <= 0) {
      alert("يرجى ملء الحقول الأساسية: العلامة التجارية، الموديل، والسعر");
      return;
    }

    let cars = this.getCars();
    if (this.editingCarId) {
      const idx = cars.findIndex(c => c.id === this.editingCarId);
    ...
    this.closeCarForm();
    this.renderCarsList();
    this.syncToMainApp(cars);
  },

  editCar(carId) {
    const cars = this.getCars();
    const car = cars.find(c => c.id === carId);
    if (car) this.showCarForm(car);
  },

  deleteCar(carId) {
    if (!confirm("هل أنت متأكد من حذف هذه السيارة؟")) return;
    let cars = this.getCars().filter(c => c.id !== carId);
    this.saveCars(cars);
    this.renderCarsList();
    this.syncToMainApp(cars);
  },
